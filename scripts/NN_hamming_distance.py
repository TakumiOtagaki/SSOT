...
def train_model(n,  y_seq, x_init, lr, num_epochs, noise_scale, dim, T, ReluThreshold, gaussian_sigma):
    ...
    for epoch in range(num_epochs):
        ...
        # 内積
        # L_X とかを一次元ベクトルに
        print("L_old dot L_new", torch.dot(L_X_old.view(-1),
              L_X.view(-1)) / (torch.norm(L_X_old) * torch.norm(L_X)))
        ...
        if torch.isnan(X).any():
            ...
            print("L_phi:", pd.DataFrame(L_phi.detach().numpy()))
            sys.exit()
...
def main():
    ...
    lr = 1e-3
    gaussian_sigma = 1.0  # 不要なので None にしておく。今後使うかもなので一応残しておくけど。
    y = "GGGAAAAAACCC"
    m = len(y)
    n = m
    x_init = "GGGAAACCCAAA"
    ...
    plt.legend()
    plt.savefig("/Users/ootagakitakumi/Library/Mobile Documents/com~apple~CloudDocs/大学院/浅井研究室/InverseFolding/SSOT/figures/L_Lprime.png")
