...
class StochasticNussinov:
    def __init__(self, rna_seq_matrix):
        ...
        self.rna_partial_sum = [sum(rna_seq_matrix[i]) for i in range(len(rna_seq_matrix))]
        self.dp = [[0] * len(rna_seq_matrix) for _ in range(len(rna_seq_matrix))]
        self.strcture = set()
...
class Rnaseq:
    ...
    def add_noise(self, noise_level=0.1):
        ...
        self.rna_seq_matrix = [[max(0, min(1, x)) for x in row] for row in self.rna_seq_matrix]
