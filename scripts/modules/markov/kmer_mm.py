...
class KmerMarkovModel:
    def __init__(self, k, pi, transition_scores):
        ...
        if k > 5:
            print(
                "Warning: The number of states grows exponentially with k. Use with caution.")
    ...
    def MAP_sequence(self, n):
        ...
        if n < self.k:
            raise ValueError(
                "n must be at least k to form a complete sequence.")
        ...
        for j in range(n - self.k, -1, -1):
            ...
            for b in range(4):
                ...
                optimal_seq_matrix[j, b] = self.transition_scores[end_state, b]
                # print("optimal_seq_matrix[j, b]:", optimal_seq_matrix[j, b])
            optimal_seq_matrix[j] = optimal_seq_matrix[j] / \
                np.sum(optimal_seq_matrix[j])
